    ...
    $.fn.imageUploader = function (options) {
        ...
        let createContainer = function () {
            ...
            let $uploadedContainer = $('<div>', { class: 'uploaded' }).appendTo($container),
               ...
               //  $i = $('<i>', { class: 'iui-cloud-upload' }).appendTo($textContainer),
                $dd = $('<div>', { class: 'aa', id: 'imm' }).appendTo($textContainer),
        ...
        let createImg = function (src, id, preloaded) {
            ...
            let $container = $('<div>', { class: 'uploaded-image' }),
                ...
                // Create the img tag
                $img = $('<img>', { src: src, style: "width:130px; height:125px;", id: 'photo' }).appendTo($container),
        ...
        let validateExtension = function (file) {
            ...
            if (plugin.settings.extensions.indexOf(file.name.replace(new RegExp('^.*\\.'), '.')) < 0) {
                ...
                if (seesion == "ar-EG") {
                    ...
                    $("#modelbody").text(`"${plugin.settings.extensions.join('", "')}"هذا الملف لا يطابق ملحقات الملفات المقبولة`);
                    $('#exampleModal').modal('show');
                }
                else {
                    $("#modelbody").text(`The file "${file.name}" does not match with the accepted file extensions: "${plugin.settings.extensions.join('", "')}"`)
                    $('#exampleModal').modal('show');
        ...
        let validateMIME = function (file) {
            ...
            if (plugin.settings.mimes.indexOf(file.type) < 0) {
                if (seesion == "ar-EG") {
                    ...
                    $("#modelbody").text(`الملف "${file.name}" لايمكن اضافته لأن الحد الاقصى من الصور هو ${plugin.settings.maxFiles}`);

                    $('#exampleModal').modal('show');
                }
                else {
                    $("#modelbody").text(`The file "${file.name}" could not be added because the limit of ${plugin.settings.maxFiles} files was reached`)
                    $('#exampleModal').modal('show');
        ...
        let validateMaxSize = function (file) {
            ...
            if (file.size > plugin.settings.maxSize) {
                if (seesion == "ar-EG") {
                    ...
                    $("#modelbody").text(`${plugin.settings.maxSize / 1024 / 1024}Mb هذا الملف تجاوز الحد الأقصى للحجم `);
                    $('#exampleModal').modal('show');
                }
                else {
                    $("#modelbody").text(`The file "${file.name}" exceeds the maximum size of ${plugin.settings.maxSize / 1024 / 1024}Mb`)
                    $('#exampleModal').modal('show');
